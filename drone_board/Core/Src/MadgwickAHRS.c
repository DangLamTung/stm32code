...
#define betaDef		0.3		// 2 * proportional gain
...
void MadgwickAHRSupdateIMU(float gx, float gy, float gz, float ax, float ay, float az) {
	...
	magd_r = magd_r*180/PI;
	magd_p = magd_p*180/PI;
	magd_y = magd_y*180/PI;
}
...
void quaternion_rot(float gx, float gy, float gz, float ax, float ay, float az,float dt){
	...
	qd0 = 0.5f * (-qr1 * gx - qr2 * gy - qr3 * gz);
	qd1 = 0.5f * (qr0 * gx + qr2 * gz - qr3 * gy);
	qd2 = 0.5f * (qr0 * gy - qr1 * gz + qr3 * gx);
	qd3 = 0.5f * (qr0 * gz + qr1 * gy - qr2 * gx);
	...
	float recipNorm;
		...
		if(!((ax == 0.0f) && (ay == 0.0f) && (az == 0.0f))) {
				...
				// Gradient decent algorithm corrective step
                f0 = 2*(qr1*q3 - qr0*qr2) - ax;
                f1 = 2*(qr0*qr1 + qr2*qr3) - ay;
                f2 = 1 - 2*(qr1*qr1 + qr2*qr2) - az;

				// Apply feedback step
				s0 = 2*(-qr2*f0 + qr1*f1);
				s1 = 2*(qr3*f0 + qr0*f1 - 2*qr1*f2);
				s2 = 2*(-qr0*f0 + qr3*f1 - 2*qr2*f2);
				s3 = 2*(qr1*f0 + qr2*f1);
				recipNorm = invSqrt(s0 * s0 + s1 * s1 + s2 * s2 + s3 * s3);
				s0 *= recipNorm;
				s1 *= recipNorm;
				s2 *= recipNorm;
				s3 *= recipNorm;// normalise
			}
