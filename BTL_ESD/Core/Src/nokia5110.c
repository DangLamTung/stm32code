...
void LCD_write_ver(uint8_t data, uint8_t mode){
  if(mode == LCD_COMMAND){
    HAL_GPIO_WritePin(lcd_gpio.DCPORT, lcd_gpio.DCPIN, GPIO_PIN_RESET);
    HAL_GPIO_WritePin(lcd_gpio.CEPORT, lcd_gpio.CEPIN, GPIO_PIN_RESET);
    LCD_send(data);
...
void LCD_constrast(uint8_t Cons){
  ...
  LCD_write(Cons, LCD_COMMAND); //set LCD Vop(Contrast).
...
void LCD_putChar_big(char c){
  for(int i = 0; i < 22; i++){
    if(lcd.inverttext != 1)
      LCD_write(Terminal11x16[(c - 0x20)*22 + i], LCD_DATA);
//    else
//      LCD_write(~(Terminal11x16[(c - 0x20)*22 + i]), LCD_DATA);
  }
}
...
//  for(int i = 56; i >0; i--){
//    if(lcd.inverttext != 1)
//      LCD_write(big_font[num][i], LCD_DATA);
//    else
//      LCD_write(~(big_font[num][i]), LCD_DATA);
//  }
//}
...
void LCD_print_num(uint8_t num, uint8_t x, uint8_t y){
  LCD_goXY(x, y);
  LCD_putNum(num);
