...
void LCD_set_pixel_remap(uint8_t x, uint8_t y, uint8_t pixel){
	   for(int k = 0; k <3 ; k ++){
	     for(int h = 0; h <3 ; h ++){
	    	 if(pixel){
	           LCD_set_pixel(3*x+k, 3*y+h, 1);
	           buffer_map[x*28+ y] = 1;
	    	 }
	    	 else{
	    		 LCD_set_pixel(3*x+k, 3*y+h, 0);
	    		 buffer_map[x*28+ y] = 0;
	    	 }
	     }
	  }
}
void createShape(char shape[9],uint8_t mode){
	 for(int k = 0; k <3 ; k ++){
		     for(int h = 0; h <3 ; h ++){
		    	 if(shape[k*3+h] == 1)
		          LCD_set_pixel_remap(k, h, mode);
		     }
		  }
}

void moveShape(char shape[9],uint8_t move){
	 for(int k = 0; k <3 ; k ++){
		     for(int h = 0; h <3 ; h ++){
		    	 if(shape[k*3+h] == 1)
		    		 createShape(shape,1);
...
void moveX(uint8_t move){
	uint8_t i = 1;
		while(i < 28){
		 LCD_set_pixel_remap(i,5, 1);
		 HAL_Delay(100);
		 LCD_printBuffer(buffer);
		 LCD_set_pixel_remap(i-1,5, 0);
	//	 HAL_Delay(100);
		 i++;
		}
}

void moveY(uint8_t move){
	uint8_t i = 1;
		while(i < 28){
		 LCD_set_pixel_remap(10,i, 1);
		 HAL_Delay(100);
		 LCD_printBuffer(buffer);
		 LCD_set_pixel_remap(10,i, 0);
	//	 HAL_Delay(100);
		 i++;
		}
}
	      ...
	      if(bar_pos>15 && bar_pos<23){
	         ...
	         if(bird_pos > bar_height -3 && bird_pos < bar_height + 3  ){
	        	 ...
	        	 while(1){
	        		 ...
	        		 char buffer_[5];
	        		 //		  char high[5];
	        		 itoa(score,buffer_, 10);
	        		 Flash_ProgramPage(buffer,DATA_START_ADDRESS,LENGTH_START_ADDRESS);
