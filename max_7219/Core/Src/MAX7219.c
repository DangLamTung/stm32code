 ...
 void MAX7219_SendByte (uchar dat)
{
    ...
    for (i=0;i<8;i++)
    {
        if((dat&0x80)==0x80) HAL_GPIO_WritePin(led_port, MAX7219_DIN,GPIO_PIN_SET);
        else HAL_GPIO_WritePin(led_port, MAX7219_DIN,GPIO_PIN_RESET);
        HAL_GPIO_WritePin(led_port, MAX7219_CLK,GPIO_PIN_RESET);
        HAL_GPIO_WritePin(led_port, MAX7219_CLK,GPIO_PIN_SET);
        dat<<=1;
    }
}
...
{
    HAL_GPIO_WritePin(led_port, MAX7219_CS,GPIO_PIN_RESET);
    MAX7219_SendByte (addr);
    MAX7219_SendByte (dat);
    HAL_GPIO_WritePin(led_port, MAX7219_CS,GPIO_PIN_SET);
}
...
void MAX7219_DisplayTime1 (uint val)
{
    uint a;
...
void MAX7219_DisplayTime2 (uint val)
{
	 uint a;

	    a=val;

MAX7219_SendAddrDat(0x08,LEDcode[(a/1000)]);
   a=a%1000;
   MAX7219_SendAddrDat(0x07,LEDcode[(a/100)]);
   a=a%100;
   MAX7219_SendAddrDat(0x06,LEDcode[(a/10)]);
   a=a%10;
   MAX7219_SendAddrDat(0x05,LEDcode[(a)]);
