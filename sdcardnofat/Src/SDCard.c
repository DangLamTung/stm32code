...
uint32_t getSize() {
	...
	if (_sdSize == 0) {
		...
		if (csd.csd_ver == 0) {
			...
			uint16_t c_size = (csd.c_size_high << 10)
					| (csd.c_size_mid << 2) | csd.c_size_low;
			uint8_t c_size_mult = (csd.c_size_mult_high << 1)
					| csd.c_size_mult_low;
...
uint8_t writeBlock(uint32_t blockaddr, uint8_t* buffer) {
