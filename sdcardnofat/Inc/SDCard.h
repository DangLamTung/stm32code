...
#define _cs1Port GPIOA
#define _cs1Pin GPIO_PIN_8
...
uint8_t initalize(); //startup the cards
uint32_t getSize(); //get the total storage space size
uint8_t readBlock(uint32_t blockaddr, uint8_t* buffer); //reads a single 512 byte block
uint8_t writeBlock(uint32_t blockaddr, uint8_t* buffer); //writes a single 512 byte block
